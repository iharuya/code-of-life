...
function mutateCode(code: string): string {
  ...
  const mutatedLines = codeLines.map((line) => {
    if (Math.random() < mutationRate) {
      ...
      return line.substring(0, insertIndex) +
        String.fromCharCode(randomCharCode) + line.substring(insertIndex);
    } else {
      return line;
    }
  });
...
function crossoverCode(code1: string, code2: string): string {
  ...
  const crossoverPoint = Math.floor(Math.random() *
    (Math.min(codeLines1.length, codeLines2.length) - 1)) + 1;
  const newCodeLines = [
    ...codeLines1.slice(0, crossoverPoint),
    ...codeLines2.slice(crossoverPoint),
  ];
...
async function evolveCode() {
  ...
  for (let generation = 1; generation <= maxGenerations; generation++) {
    ...
    // 評価
    const scores = await Promise.all(population.map(evaluateCode));
