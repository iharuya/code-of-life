...
const systemPrompt =
	"あなたは、与えられたコードを改善し、新しい機能を追加する天才プログラマーです。自己言及やコードの説明はせず、コードのみを出力してください。"
const nextInstruction =
	"## Instruction\n- `--name`オプションを追加してください。\n- `--name`オプションで指定された名前のファイルを生成するようにしてください。\n- ファイル名はログファイル名生成処理にも適用してください。"

const nextGeneration = response.replace(
	/```(?:ts|typescript)\s*/,
	`\`\`\`typescript
${systemPrompt}
${nextInstruction}
