...
const extractCode = (text) => {
	const match = text.match(/```(?:ts|typescript)\s*([\s\S]+?)\s*```/)
	return match ? match[1].trim() : null
}

const extractedCode = extractCode(response)
if (extractedCode) {
	const result = executeJavaScript(extractedCode)
	console.log("コードの実行結果:", result)
}
 ...
 *
 * Instruction
 * コードの実行結果をもとに、次の世代のコードを生成するようにしてください。
 * 例えば、実行結果が数値なら、その数値を使って何か面白いことを実装してみてください。
 */
